...
class NoUTurnSampler(kernel.TransitionKernel):
  ...
  def _start_trajectory_batched(self, state, target_log_prob, seed):
    ...
    Args:
        state: [theta, u] joint state
        target_log_prob: target function value
        seed: random seed
        
    Returns:
        momentum: [rho, p] joint momentum
        init_energy: initial energy
        log_slice_sample: slice sampling variable
    """
    with tf.name_scope('start_trajectory_batched'):
        theta, u = state  # unpack state
        seeds = samplers.split_seed(seed, n=3)
        theta_seed, u_seed, slice_seed = seeds
        
        # generate momentum of theta
        rho = samplers.normal(
            shape=ps.shape(theta),
            dtype=theta.dtype,
            seed=theta_seed) * tf.sqrt(self._rho_size)
        
        # generate momentum of u
        p = samplers.normal(
            shape=ps.shape(u),
            dtype=u.dtype,
            seed=u_seed)
        ...
        init_energy = compute_hamiltonian(
            target_log_prob, momentum, state,
            self._rho_size,
            shard_axis_names=self.experimental_shard_axis_names)
  ...
  def _loop_tree_doubling(self, step_size, momentum_state_memory,
    ...
    with tf.name_scope('loop_tree_doubling'):
        ...
        # use PMLeapfrogIntegrator instead of SimpleLeapfrogIntegrator
        integrator = PMLeapfrogIntegrator(
            ...
            rho_size=self._rho_size)  # pass rho_size parameter
        ...
        # because MULTINOMIAL_SAMPLE=False, use slice sampling
        ...
        # because GENERALIZED_UTURN=False, use original U-turn condition
  ...
  def _loop_build_sub_tree(self,
                         ...
                         momentum_state_memory,
                         seed):
    """basic steps of building sub-tree."""
    ...
    with tf.name_scope('loop_build_sub_tree'):
        ...
        # if tree did not terminate, count this leapfrog step
        ...
        # because GENERALIZED_UTURN=False, use original state difference
        ...
        no_u_turns_within_tree = has_not_u_turn_at_all_index(
            read_index,
            directions,
            momentum_state_memory,
            next_momentum_parts,
        ...
        # compute energy
        energy = compute_hamiltonian(
            next_target, next_momentum_parts, next_state_parts,
            self._rho_size,
            shard_axis_names=self.experimental_shard_axis_names)
...
def has_not_u_turn(state_diff, momentum_left, momentum_right, 
                   log_prob_rank, shard_axis_names=None):
    """check u turn condition.
    
    Args:
        state_diff: [theta_diff, u_diff] state difference
        momentum_left: [rho_left, p_left] left momentum
        momentum_right: [rho_right, p_right] right momentum
        log_prob_rank: probability rank
        shard_axis_names: shard axis names
        
    Returns:
        boolean value of whether u turn condition is not satisfied
    """
    shard_axis_names = (shard_axis_names or ([None] * len(state_diff)))
    
    # unpack state difference and momentum
    theta_diff, u_diff = state_diff
    rho_left, p_left = momentum_left
    rho_right, p_right = momentum_right
    
    def reduce_sum(x, m, axes):
        out = tf.reduce_sum(x * m, axis=ps.range(log_prob_rank, ps.rank(m)))
        if axes is not None:
            out = distribute_lib.psum(out, axes)
        return out
    
    # compute dot product of theta
    theta_dot_left = reduce_sum(theta_diff, rho_left, shard_axis_names[0])
    theta_dot_right = reduce_sum(theta_diff, rho_right, shard_axis_names[0])
    
    # compute dot product of u
    u_dot_left = reduce_sum(u_diff, p_left, shard_axis_names[1])
    u_dot_right = reduce_sum(u_diff, p_right, shard_axis_names[1])
    
    # compute total dot product
    total_dot_left = theta_dot_left + u_dot_left
    total_dot_right = theta_dot_right + u_dot_right
    
    return (total_dot_left >= 0) & (total_dot_right >= 0)
...
def compute_hamiltonian(target_log_prob, momentum_parts, state_parts=None, 
                       rho_size=10.0, shard_axis_names=None):
    """Hamiltonian for PM-NUTS.
    
    Args:
        target_log_prob: target function value
        momentum_parts: [rho, p] momentum
        state_parts: [theta, u] state
        rho_size: scaling factor of theta momentum
        shard_axis_names: shard axis names
    ...
    rho, p = momentum_parts  # unpack momentum
    if state_parts is not None:
        _, u = state_parts  # unpack state
    ...
    # compute kinetic energy of rho
    rho_kinetic = 0.5 * compute_sum_sq(rho, shard_axis_names[0]) / rho_size
    
    # compute kinetic energy of p
    p_kinetic = 0.5 * compute_sum_sq(p, shard_axis_names[1])
    
    # compute kinetic energy of u (if state_parts is provided)
    u_kinetic = 0.5 * compute_sum_sq(u, shard_axis_names[1]) if state_parts is not None else 0.
    
    return target_log_prob - (rho_kinetic + p_kinetic + u_kinetic)
