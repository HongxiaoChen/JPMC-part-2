...
from tfp_modified_kernels.NUTS import NoUTurnSampler
from log_likelihood_auto import compute_log_likelihood_and_gradients_auto, compute_log_posterior
from params import NUTS_PARAMS, MCMC_PARAMS, DATA_PARAMS, initialize_theta
...
def run_pm_nuts_sampling():
    """
    Run PM-NUTS sampling.
    """
    # Initialize sample data
    ...
    num_results = NUTS_PARAMS['total_samples']  # Number of samples
    num_burnin_steps = NUTS_PARAMS['burn_in']  # Burn-in steps
    h = NUTS_PARAMS['nuts_step_size']  # Step size
    rho_size = NUTS_PARAMS['rho_size']  # rho_size parameter
    
    # Create PM-NUTS kernel
    print(f"\nSampling using PM-NUTS...")
    print(f"Step size h = {h}, rho_size = {rho_size}")
    ...
    # Set main title
    plt.suptitle(
        f'PM-NUTS Sampling Results\n'
        f'N = {N}, h = {h}, ρ mass = {rho_size}\n',
        fontsize=14
    )
    ...
    plt.savefig(f'figures/TFP_NUTS_parameter_traces_{timestamp}.png', dpi=300, bbox_inches='tight')
    
    print(f"Figure saved to figures/TFP_NUTS_parameter_traces_{timestamp}.png")
...
if __name__ == '__main__':
    # Run PM-NUTS sampling
    theta_samples = run_pm_nuts_sampling()
    
    # Print results summary
    print("\nPM-NUTS Sampling Summary:")
